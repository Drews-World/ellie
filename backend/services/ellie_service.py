...
async def get_ellie_brief(widget: str, context: dict = {}) -> str:
    ...
    # Inject context if provided (e.g. user's actual calendar events)
    if context:
        context_str = "\n\nContext data:\n" + str(context)
    ...
    try:
        ...
        if 'credit balance is too low' in err or 'billing' in err.lower():
            return "**⚠ ELLIE OFFLINE** — API credits exhausted. Add credits at console.anthropic.com/settings/billing. — ELLIE"
        raise
...
async def ellie_chat(messages: list, context: dict = {}) -> str:
    ...
    # Pattern recognition & cross-domain intelligence layer
    system += """
...
- Cross-Domain Connections: Actively connect dots between Drew's world — if a Fed rate decision affects Quill's fundraising runway, say so. If a geopolitical event impacts Amazon's supply chain or his portfolio, flag it.
- Proactive Alerts: If something in Drew's context warrants urgent attention (an overdue goal, a conflict affecting something he cares about, a market move near a threshold), lead with it rather than waiting to be asked.
- Stay concise. Surface the insight, not the reasoning."""
    ...
    try:
        ...
        err = str(e)
        if 'credit balance is too low' in err or 'billing' in err.lower():
            return "**⚠ ELLIE OFFLINE** — Anthropic API credits exhausted. Add credits at console.anthropic.com/settings/billing, then update ANTHROPIC_API_KEY in backend/.env and restart. — ELLIE"
        if 'invalid_api_key' in err or 'authentication' in err.lower():
            return "**⚠ ELLIE OFFLINE** — Invalid Anthropic API key. Update ANTHROPIC_API_KEY in backend/.env. — ELLIE"
        raise
