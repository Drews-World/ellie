...
export default function BrokerageView() {
  ...
  const load = async () => {
    ...
    try {
      const [acc, pos, ord, cfg] = await Promise.all([
        tfetch('/alpaca/account').then(r => r.json()),
        tfetch('/alpaca/positions').then(r => r.json()),
        tfetch('/alpaca/orders').then(r => r.json()),
        tfetch('/alpaca/config').then(r => r.json()),
      ])
      setAccount(acc)
  ...
  const submitOrder = async () => {
    ...
    setOrderMsg(null)
    try {
      const res = await tfetch('/alpaca/order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const closePosition = async (symbol) => {
    ...
    await tfetch(`/alpaca/positions/${symbol}`, { method: 'DELETE' })
