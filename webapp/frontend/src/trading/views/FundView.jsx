...
export default function FundView() {
  ...
  const load = useCallback(async () => {
    try {
      const [f, acc, lg] = await Promise.all([
        tfetch('/fund').then(r => r.json()),
        tfetch('/alpaca/account').then(r => r.json()).catch(() => null),
        tfetch('/fund/log').then(r => r.json()),
      ])
      setFund(f)
      setAccount(acc)
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      const r = await tfetch('/fund/launch', { method: 'POST' })
  ...
  const handlePause = async () => {
    setBusy(true)
    try {
      await tfetch('/fund/pause', { method: 'POST' })
      showMsg('Fund paused.')
      load()
  ...
  const handleResume = async () => {
    setBusy(true)
    try {
      await tfetch('/fund/resume', { method: 'POST' })
  ...
  const handleDiscover = async () => {
    ...
    setBusy(true)
    try {
      const r = await tfetch('/fund/discover', { method: 'POST' })
      const d = await r.json()
      if (d.ok) {
  ...
  const handleReset = async () => {
    ...
    setBusy(true)
    try {
      const r = await tfetch('/fund/reset', { method: 'POST' })
      const d = await r.json()
      if (d.ok) {
