...
function CatalystWatch({ fund }) {
  ...
  const loadCatalysts = useCallback(async () => {
    try {
      const r = await tfetch('/fund/catalysts')
  ...
  const handleScan = async () => {
    setScanning(true)
    try {
      await tfetch('/fund/catalysts/scan', { method: 'POST' })
  ...
  const handleTrigger = async (id) => {
    await tfetch(`/fund/catalysts/${id}/trigger`, { method: 'POST' })
    setTimeout(loadCatalysts, 2000)
  }

  const handleDelete = async (id) => {
    await tfetch(`/fund/catalysts/${id}`, { method: 'DELETE' })
...
function BuyBacklog({ fundConfig, onConfigChange }) {
  ...
  const loadBacklog = useCallback(async () => {
    try {
      const r = await tfetch('/fund/backlog')
      if (r.ok) {
  ...
  const handleBuy = async (id) => {
    ...
    try {
      const r = await tfetch(`/fund/backlog/${id}/buy`, { method: 'POST' })
      const d = await r.json()
      if (d.ok) loadBacklog()
    } finally {
  ...
  const handleRemove = async (id) => {
    await tfetch(`/fund/backlog/${id}`, { method: 'DELETE' })
    setItems(prev => prev.filter(i => i.id !== id))
  }

  const handleClearAll = async () => {
    if (!confirm('Clear all pending backlog buys?')) return
    await tfetch('/fund/backlog/clear', { method: 'POST' })
    loadBacklog()
  }
...
function FundAutomations({ fund, onRunReview }) {
  ...
  const handleRunReview = async () => {
    setReviewing(true)
    try {
      await tfetch('/fund/review', { method: 'POST' })
      onRunReview()
    } finally {
      setReviewing(false)
    }
...
export default function MonitorView() {
  ...
  const handleTestDiscord = async () => {
    ...
    setTestResult(null)
    try {
      const r = await tfetch('/discord/test', { method: 'POST' })
  ...
  const handleDelete = async (id) => {
    await tfetch(`/monitor/${id}`, { method: 'DELETE' })
  ...
  const handleRunNow = async (id) => {
    ...
    try {
      await tfetch(`/monitor/${id}/run`, { method: 'POST' })
      await load(true)
    } finally {
  ...
  const markAllRead = async () => {
    await tfetch('/monitor/alerts/read-all', { method: 'POST' })
